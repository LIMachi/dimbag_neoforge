...
public class SensorMenu extends AbstractContainerMenu implements IAcceptUpStreamNBT {
    @RegisterMenu
    ...
    public static void open(Player player, BlockPos pos) {
        if (!player.level().isClientSide) {
            BagsData.runOnBag(player.level(), pos, b->{
                CompoundTag data = b.getModule(SensorModule.NAME, pos);
                NetworkHooks.openScreen((ServerPlayer) player, new SimpleMenuProvider((id, inv, p) -> new SensorMenu(id, inv, data), Component.Serializer.fromJson(data.getString("label"))), buff -> buff.writeNbt(data));
            });
        }
    }

    public SensorMenu(int id, Inventory playerInventory, FriendlyByteBuf buff) {
        this(id, playerInventory, buff.readNbt());
    }

    public final CompoundTag data;

    public SensorMenu(int id, Inventory playerInventory, CompoundTag data) {
        super(R_TYPE.get(), id);
        this.data = data.copy();
    }
    ...
    public void upstreamNBTMessage(int i, CompoundTag compoundTag) {
        ...
        BagsData.runOnBag(World.getLevel(DimBag.BAG_DIM), pos, b->b.getModule(SensorModule.NAME, pos).merge(compoundTag));
    }
}
